...
export default function NBAPlayoffPool({ dbPath, poolId, adminAuthed, onAdminLogin }) {
  ...
  const [myName,    setMyName]    = useState(() => localStorage.getItem(`${poolId}_pool_name`) || localStorage.getItem("pool_name") || "");
  const [myName2,   setMyName2]   = useState(() => localStorage.getItem(`${poolId}_pool_name_2`) || localStorage.getItem("pool_name_2") || "");
  ...
  useEffect(() => {
    ...
    const unsubResults = onValue(ref(db, `${dbPath}/results`), snap => {
    ...
    const unsubParticipants = onValue(ref(db, `${dbPath}/participants`), snap => {
      setParticipants(snap.exists() ? snap.val() : {});
    });

    // Listen to picks lock state
    const unsubLock = onValue(ref(db, `${dbPath}/settings/picksLocked`), snap => {
      setPicksLocked(snap.exists() ? snap.val() : false);
    });

    // Listen to submission deadline (set by admin)
    const unsubDeadline = onValue(ref(db, `${dbPath}/settings/deadline`), snap => {
    ...
    const unsubPlayIn = onValue(ref(db, `${dbPath}/settings/playInResults`), snap => {
    ...
    const savedName = localStorage.getItem(`${poolId}_pool_name`) || localStorage.getItem("pool_name");
    if (savedName) {
      const unsubMe = onValue(ref(db, `${dbPath}/participants/${sanitize(savedName)}/picks`), snap => {
        if (snap.exists()) setMyPicks(snap.val());
      });
      const unsubMe2 = onValue(ref(db, `${dbPath}/participants/${sanitize(savedName)}/picks2`), snap => {
        if (snap.exists()) setMyPicks2(snap.val());
      });
      const unsubPIP = onValue(ref(db, `${dbPath}/participants/${sanitize(savedName)}/playInPicks`), snap => {
        if (snap.exists()) setPlayInPicks(snap.val());
      });
      const unsubPIP2 = onValue(ref(db, `${dbPath}/participants/${sanitize(savedName)}/playInPicks2`), snap => {
  ...
  const handleSubmit = async () => {
    ...
    if (activeEntry === 1) {
      ...
      const prevKey = sanitize(localStorage.getItem(`${poolId}_pool_name`) || localStorage.getItem("pool_name") || "");
    ...
    try {
      ...
      if (activeEntry === 1) {
        ...
        await set(ref(db, `${dbPath}/participants/${key}`), {
          name:         myName.trim(),
          email:        myEmail.trim(),
          picks:        myPicks,
        ...
        localStorage.setItem(`${poolId}_pool_name`,      myName.trim());
        ...
        await update(ref(db), {
          [`${dbPath}/participants/${key}/picks2`]:       myPicks2,
          [`${dbPath}/participants/${key}/playInPicks2`]: Object.keys(playInPicks2).length > 0 ? playInPicks2 : null,
          [`${dbPath}/participants/${key}/submittedAt2`]: Date.now(),
          [`${dbPath}/participants/${key}/name2`]:        name2Value,
          [`${dbPath}/participants/${key}/email2`]:       email2Value,
        });
        localStorage.setItem(`${poolId}_pool_submitted_2`, "1");
        localStorage.setItem(`${poolId}_pool_name_2`,  name2Value);
        localStorage.setItem(`${poolId}_pool_email_2`, myEmail2.trim());
        setSubmitted2(true);
        // First-time submit → show success overlay; update → just toast
        if (wasFirstE2) {
...
;
  ...
  const handleAdminPlayIn = async (gameId, winner) => {
    try {
      if (winner) {
        await set(ref(db, `${dbPath}/settings/playInResults/${gameId}`), winner);
      } else {
        // Clear the result
        await set(ref(db, `${dbPath}/settings/playInResults/${gameId}`), null);
      }
  ...
  const handleToggleLock = async () => {
    try {
      await set(ref(db, `${dbPath}/settings/picksLocked`), !picksLocked);
  ...
  const handleSaveDeadline = async () => {
    ...
    const isoStr = adminDeadlineTime
      ...
      : adminDeadlineDate;
    try {
      await set(ref(db, `${dbPath}/settings/deadline`), isoStr);
  ...
  const handlePaidToggle = async (participantKey, currentPaid) => {
    try {
      const updates = {};
      updates[`${dbPath}/participants/${participantKey}/paid`] = !currentPaid;
      ...
      <div className="app">
        ...
        {tab === "admin" && !adminAuthed && (
          <div className="admin-gate">
            ...
            <div className="admin-gate-sub" style={{lineHeight:1.6}}>
              Sign in via <strong style={{color:"var(--text2)"}}>Admin Hub</strong> in the left menu to access admin controls.
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{display:'flex', justifyContent:'flex-end', alignItems:'center',
              ...
              <button className="btn" style={{fontSize:'0.72rem', padding:'5px 14px'}}
                onClick={async () => { try { const { signOut } = await import("firebase/auth"); if (auth) await signOut(auth); } catch(e) { console.error(e); } }}>
                Sign Out
              </button>
            </div>
            {/* Lock toggle */}
